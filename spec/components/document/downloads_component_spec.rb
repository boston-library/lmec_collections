...
RSpec.describe Document::DownloadsComponent, type: :component do
  ...
  context 'non-restricted institution' do
  ...
  context 'restricted institution' do
    let(:document) { SolrDocument.new(id: item_pid, license_ss: 'Creative Commons', institution_name_ssi: 'British Library') }

    it 'renders the component' do
      render_inline(described_class.new(document: document, object_files: files_hash))

      expect(page).to have_css('li.download_list_item a.sidebar_downloads_link', count: 2)
      expect(page).to_not have_link(I18n.t('blacklight.downloads.images.image_primary'), href: "/downloads/#{item_pid}?filestream_id=image_primary")
    end
  end
end
